...
ExpMod::ExpMod(Value* val1, Value* val2):Expression(val1, val2) {
    this->clobber_counter = 5;
}

vector<string> ExpMod::getCode(SymbolTable* symbolTable, RegManager* regManager) {
    vector<string> code;
    ...
    // save number of numerator bits we will need it to restore remainder at the end
    code.push_back("RESET " + this->clobbers.at(4));
    code.push_back("ADD " + this->clobbers.at(4) + " " + this->outcome_reg);
